...
pub struct SliderGraphics<T> {
    pub bar: Vec<T>,
    pub handle: T,
    pub handle_clicked: T,
}

impl<'a> From<SliderGraphics<&'a LoadedTexture>> for SliderGraphics<eframe::egui::widgets::Image> {
    fn from(textures: SliderGraphics<&'a LoadedTexture>) -> Self {
        Self {
            bar: textures.bar.iter().map(|tex| eframe::egui::widgets::Image::new(tex.texture, tex.size.clone())).collect(),
            handle: eframe::egui::widgets::Image::new(textures.handle.texture, textures.handle.size.clone()),
            handle_clicked: eframe::egui::widgets::Image::new(textures.handle_clicked.texture, textures.handle_clicked.size.clone()),
        }
    }
}
...
pub struct WinampSlider<'a> {
    get_set_value: GetSetValue<'a>,
    range: RangeInclusive<f64>,
    spec: WinampSliderSpec,
    clamp_to_range: bool,
    smart_aim: bool,
    show_value: bool,
    prefix: String,
    ...
    image_bundle: SliderGraphics<eframe::egui::widgets::Image>
}

macro_rules! impl_integer_constructor {
    ($int:ident) => {
        #[deprecated = "Use Slider::new instead"]
        pub fn $int(value: &'a mut $int, range: RangeInclusive<$int>, textures: SliderGraphics<&LoadedTexture>) -> Self {
...
impl<'a> WinampSlider<'a> {
    pub fn new<Num: emath::Numeric>(value: &'a mut Num, range: RangeInclusive<Num>, textures: SliderGraphics<&LoadedTexture>) -> Self {
    ...
    #[deprecated = "Use Slider::new instead"]
    pub fn f32(value: &'a mut f32, range: RangeInclusive<f32>, textures: SliderGraphics<&LoadedTexture>) -> Self {
        let range_f64 = (*range.start() as f64)..=(*range.end() as f64);
        Self::from_get_set(range_f64, move |v: Option<f64>| {
            if let Some(v) = v {
                *value = v as f32
    ...
    #[deprecated = "Use Slider::new instead"]
    pub fn f64(value: &'a mut f64, range: RangeInclusive<f64>, textures: SliderGraphics<&LoadedTexture>) -> Self {
    ...
    pub fn from_get_set(
        ...
        textures: SliderGraphics<&LoadedTexture>
    ) -> Self {
        Self {
...
impl<'a> WinampSlider<'a> {
    ...
    fn allocate_slider_space(&self, ui: &mut Ui, height: f32) -> Response {
        let desired_size = self.image_bundle.bar.first().unwrap().size();
    ...
    fn slider_ui(&mut self, ui: &mut Ui, response: &Response) {
        ...
        // Paint it:
        {
            ...
            let rail_image_index = (value * (self.image_bundle.bar.len() as f64 - 1.0)).round() as usize;
            println!("index {}", rail_image_index);
            let rail_image = self.image_bundle.bar.get(rail_image_index).unwrap(); // double check the math here.
            rail_image.paint_at(ui, *rect);
            let marker_center_x = self.x_from_value(value, x_range);

            let handle_image = self.image_bundle.handle;
            let handle_image_rect = Rect::from_center_size(pos2(marker_center_x, rect.center().y), handle_image.size());
            handle_image.paint_at(ui, handle_image_rect);
        }
    }
...
impl<'a> Widget for WinampSlider<'a> {
...
fn value_from_normalized(normalized: f64, range: RangeInclusive<f64>, spec: &WinampSliderSpec) -> f64 {
...
fn normalized_from_value(value: f64, range: RangeInclusive<f64>, spec: &WinampSliderSpec) -> f64 {
    let (min, max) = (*range.start(), *range.end());
...
fn range_log10(min: f64, max: f64, spec: &WinampSliderSpec) -> (f64, f64) {
...
fn logaritmic_zero_cutoff(min: f64, max: f64) -> f64 {
    ...
    eframe::egui::egui_assert!(0.0 <= cutoff && cutoff <= 1.0);
    cutoff
}
